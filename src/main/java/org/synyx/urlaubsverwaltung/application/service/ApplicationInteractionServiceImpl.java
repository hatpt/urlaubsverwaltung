...
public class ApplicationInteractionServiceImpl implements ApplicationInteractionService {
    ...
    @Override
    public Application cancelCancellationRequest(Application applicationForLeave, Person person, Optional<String> comment){

        if (applicationForLeave.getStatus().compareTo(ALLOWED_CANCELLATION_REQUESTED) != 0) {
            throw new CancelCancellationRequestedApplicationForLeaveNotAllowedException(format("Cannot cancel the cancellation " +
        ...
        commentService.create(savedApplication, CANCEL_REQUESTED_CANCELLED, comment, person);

        applicationMailService.sendCancelledCancellationRequestApplicationNotification(savedApplication, person);
